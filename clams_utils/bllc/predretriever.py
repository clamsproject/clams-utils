...
def download_preds(storage_url, pipeline_json, folder_name: os.path = None):
    import tempfile
    ...
    # note it expects a string so we use json.dumps
    response = requests.post(storage_url, json.dumps(pipeline_json), headers={"Content-Type": "application/json"})
    # read the response as json. keys are guids and values are serialized mmif files
    mmifs = json.loads(response.content)
